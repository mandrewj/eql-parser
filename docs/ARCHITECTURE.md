...
- **Recent encounters**: each slain NPC is finalized into a rolling last-N list (newest first, oldest drops) with per-character cards (self + top 5 by DPS, damage-to-mob + taken-from-mob, drilldownable). Per-(target, attacker) damage is aggregated as full metric accumulators for this.
