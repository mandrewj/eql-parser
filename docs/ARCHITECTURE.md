...
- **Recent encounters**: each NPC is finalized into a rolling last-N list (newest first, oldest drops) — on death, **or on fight close** (zone / 90s / abandon) for engaged-but-unslain mobs (a boss you fled). Per-character cards (self + top 5 by DPS) carry all three metrics: damage-to-mob (drilldownable), damage-taken-from-mob, and healing done during the encounter's time window (healing isn't mob-specific, so it's windowed). Per-(target, attacker) damage is aggregated as full metric accumulators.
