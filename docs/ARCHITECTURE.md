...
- **Recent encounters**: each NPC is finalized into a rolling last-N list (newest first, oldest drops) — on death, **or on fight close** (zone / 90s / abandon) for engaged-but-unslain mobs (a boss you fled). Per-character cards (self + top 5, ranked by DPS) carry all three metrics: damage-to-mob (drilldownable), damage-taken-from-mob, and healing done in the encounter window. **Rates are per-person**: each character's active window starts at *their* first contact with the mob (first attack, or the mob first hitting/casting on them — tracked as per-`attacker>target` first-contact timestamps) and runs to the encounter end, so late-joiners aren't diluted. Per-(target, attacker) damage is aggregated as full metric accumulators.
